...
#include "misc/addrman.h"
#include "misc/txdb.h"
#include "misc/net.h"
#include "misc/key.h"
#include "misc/pubkey.h"
#include "misc/util.h"
...
#include "darksend/darksend-relay.h"
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: AnuCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: AnuCoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 36963)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 36964)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeAnuCoinamount=<n> " + _("Keep N AnuCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. AnuCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. AnuCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("AnuCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "AnuCoin server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of AnuCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart AnuCoin to complete") << "\n";
    ...
    nAnonymizeAnuCoinAmount = GetArg("-anonymizeAnuCoinamount", 0);
    if(nAnonymizeAnuCoinAmount > 999999) nAnonymizeAnuCoinAmount = 999999;
    if(nAnonymizeAnuCoinAmount < 2) nAnonymizeAnuCoinAmount = 2;
    ...
    LogPrintf("Anonymize AnuCoin Amount %d\n", nAnonymizeAnuCoinAmount);
