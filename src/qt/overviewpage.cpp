...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeAnuCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAnuCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeAnuCoinAmount = strAnonymizeAnuCoinAmount.remove(strAnonymizeAnuCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAnuCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeAnuCoinAmount*COIN) nMaxToAnonymize = nAnonymizeAnuCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeAnuCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeAnuCoinAmount));
        strAnonymizeAnuCoinAmount = strAnonymizeAnuCoinAmount.remove(strAnonymizeAnuCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAnuCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
