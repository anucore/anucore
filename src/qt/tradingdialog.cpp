    ...
    model(0)
{
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "ANU(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "ANU(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    /*OrderBook Table Init*/

    /*Market History Table Init*/
    ui->MarketHistoryTable->setColumnCount(5);
    ui->MarketHistoryTable->verticalHeader()->setVisible(false);
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(ANU)"<<"TOTAL COST(BTC");
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://future-exchange.com/api/v1.1/public/GetMarketSummary?market=btc-ANU");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://future-exchange.com/api/v1.1/public/getorderbook?market=BTC-ANU&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://future-exchange.com/api/v1.1/public/getmarkethistory?market=BTC-ANU&count=100");
      return Response;
}
...
QString tradingDialog::BuyTX(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://future-exchange.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-ANU&quantity=";
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://future-exchange.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-ANU";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://future-exchange.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=ANU";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://future-exchange.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-ANU&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>ANU Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> ANU");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->TXSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(TXSupply,'i',8) + "</span><b> ANU</b>");
    ...
    ui->TXDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(TXDemand,'i',8) + "</span><b> ANU</b>");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("ANU");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->TXBalanceLabel,*ui->TXAvailableLabel_2,*ui->TXPendingLabel, QString::fromUtf8("ANU"),Response);
                    }
                break;
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("ANU");
...
{
    //calculate amount of BTC that can be gained from sellingANUavailable balance
    QString responseA = GetBalance("ANU");
    QString str;
                ...
                if (SellResponseObject["success"].toBool() == false){
                    ...
                    if (ResponseObject["success"].toBool() == false){
                        ...
                        if (ResponseObject["success"].toBool() == false){
                            ...
                            QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+"ANUfor "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                        }
                    } else if (ResponseObject["success"].toBool() == true){
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+"ANUfor "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
