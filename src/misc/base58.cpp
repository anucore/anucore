...
namespace {
    class CAnuCoinAddressVisitor : public boost::static_visitor<bool> {
    private:
        CAnuCoinAddress *addr;
    public:
        CAnuCoinAddressVisitor(CAnuCoinAddress *addrIn) : addr(addrIn) { }
...
bool CAnuCoinAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CAnuCoinAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CAnuCoinAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CAnuCoinAddressVisitor(this), dest);
}

bool CAnuCoinAddress::IsValid() const {
...
CTxDestination CAnuCoinAddress::Get() const {
    if (!IsValid())
...
bool CAnuCoinAddress::GetKeyID(CKeyID &keyID) const {
...
bool CAnuCoinAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CAnuCoinSecret::SetKey(const CKey& vchSecret) {
...
CKey CAnuCoinSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CAnuCoinSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CAnuCoinSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CAnuCoinSecret::SetString(const std::string& strSecret) {
