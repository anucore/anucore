...
int nAnonymizeAnuCoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AnuCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AnuCoin
    // Mac: ~/Library/Application Support/AnuCoin
    // Unix: ~/.AnuCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AnuCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "AnuCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "AnuCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong AnuCoin will not work properly.");
